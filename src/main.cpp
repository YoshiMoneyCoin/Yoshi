...
const string strMessageMagic = "YoshiCoin Signed Message:\n";
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
	...
	printf("Proof nBits = %s|%s|%s\n", bnTarget.getuint256().ToString().c_str(), hash.ToString().c_str(), hash > bnTarget.getuint256() ? "true" : "false");
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "YoshiCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "China faces being cut out of Britain’s nuclear future";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1472474860;
        }
        else
        {
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0x2306a74035e196a7658f253a50abd296f31e5739a80cdbc0eb3d537f4836ddc6"));
        block.print();

		/*for (unsigned long ii = 150000; ii < UINT64_MAX; ii++) {
			block.nNonce = ii;
			printf("Ii = %lu\n", ii);
			printf("block.GetHash() == %s\n", block.GetHash().ToString().c_str());
			printf("block.hashMerkleRoot == %s\n", block.hashMerkleRoot.ToString().c_str());
...
unsigned char pchMessageStart[4] = { 0xf3, 0xdd, 0xdd, 0xe3 };
